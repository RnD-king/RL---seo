...
def build_args():
    custom_parameters = [
        ...
        {
            "name": "--freeze-other-joints",
            "action": "store_true",
            "default": False,
            "help": "After every sim step, force all non-tested joints back to default position and zero velocity.",
        },
...
def main():
    ...
    other_joint_ids = [idx for idx in range(env.num_dof) if idx != j]
    ...
    if args.freeze_other_joints:
        print("[info] freeze_other_joints=True: non-tested joints are forced to default pos and zero velocity after every step")
    print(f"[info] csv={csv_path}")
    print(f"[info] summary={summary_path}")

    default_target = env.default_dof_pos.clone()

    def freeze_other_joints() -> None:
        """Force every non-tested joint to stay at its default pose.

        This is intentionally non-physical, but it is useful when the goal is to
        compare one real motor step response against one simulated joint response.
        """
        if not args.freeze_other_joints:
            return
        if not other_joint_ids:
            return
        ids = torch.tensor(other_joint_ids, device=env.device, dtype=torch.long)
        env.dof_pos[:, ids] = env.default_dof_pos[:, ids]
        env.dof_vel[:, ids] = 0.0
        env.gym.set_dof_state_tensor(env.sim, gymtorch.unwrap_tensor(env.dof_state))
